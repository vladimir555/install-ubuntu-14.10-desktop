...
class Firewall():
    ...
    def allowClient(self, interface_list, profile_list):
        for i in interface_list:
            for profile in profile_list:
                for rule in profile.value:
                    if "src_port" in rule:
                        self.iptables.acceptPortToPort  (i, Direction.OUTPUT, rule[Protocol], rule["dst_port"], rule["src_port"])
                    else:
                        self.iptables.acceptPort        (i, Direction.OUTPUT, rule[Protocol], rule["dst_port"])
    ...
    def allowServer(self, interface_list, profile_list):
        for i in interface_list:
            for profile in profile_list:
                for rule in profile.value:
                    if "src_port" in rule:
                        self.iptables.acceptPortToPort  (i, Direction.INPUT,  rule[Protocol], rule["src_port"], rule["dst_port"])
                    else:
                        self.iptables.acceptPort        (i, Direction.INPUT,  rule[Protocol], rule["dst_port"])

    def forward(self, interface_out, interface_in_list, profile_list):
        for i in interface_in_list:
            for profile in profile_list:
                for rule in profile.value:
                    self.iptables.forward(interface_out, i, rule[Protocol], rule["dst_port"])
    ...
    def rerouteHTTPToTransparentProxy(self, interface_in_list):
        for i in interface_in_list:
