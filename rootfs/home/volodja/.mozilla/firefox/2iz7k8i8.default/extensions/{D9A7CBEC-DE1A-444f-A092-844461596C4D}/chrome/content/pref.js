...
var gDefaultFullZoomLevelPreferences = {
  onLoad: function() {
    ...
    var win = this.getMostRecentWindow();
    if (win.document.getElementById('addon-bar'))
      document.getElementById('showinstatusbar').setAttribute('disabled', true);
  },
  ...
  onAccept: function() {
    ...
    defaultFullZoom = document.getElementById('maximumFitZoomLevel');
    try{
    ...
    gPrefService.setIntPref("extensions.browser.zoom.fitZoom.maximum",p);


    defaultFullZoom = document.getElementById('minimumFitZoomLevel');
    try{
      p = Math.floor(defaultFullZoom.value);
      if(p < 10) p = 10;
      if(p > 1000) p = 1000;
    ...
    gPrefService.setIntPref("extensions.browser.zoom.fitZoom.minimum",p);
  ...
  reset: function(){
    ...
    if (!Components.interfaces.nsIContentPrefService2) {
      ...
      cps.removePrefsByName("browser.content.full-AutoFit", null);
    } else {
      ...
      cps2.removeByName("browser.content.full-AutoFit", null);
    }
  },
