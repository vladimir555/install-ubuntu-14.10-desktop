var EXPORTED_SYMBOLS = ["fvd_speed_dial_NewsLetter"]; 

Components.utils.import("resource://fvd.speeddial.modules/settings.js");

var fvd_speed_dial_NewsLetter = new function(){
	...
	this.displayDialog = function( parent, email ){
		...
		parent.openDialog('chrome://fvd.speeddial/content/dialogs/fvd_sd_subscribe_newsletter.xul', '', 'chrome,titlebar,toolbar,centerscreen,modal', arguments);
	}
	...
	this.subscribe = function( email, name, callback ){
		...
		try{
			alreadySubscribed = JSON.parse( fvd_speed_dial_gFVDSSDSettings.getStringVal( "sd.subscribed_emails" ) );
		}
		catch( ex ){
		...
		fvd_speed_dial_gFVDSSDSettings.setStringVal( "sd.subscribed_emails", JSON.stringify( alreadySubscribed ) );
